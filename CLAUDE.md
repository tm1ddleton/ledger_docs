...
- Position: the aggregation of moves that share the same unit, wallet, and counterparty wallet.  Positions are supplied to the smart contract as input, scoped to a specific (unit, wallet, counterparty wallet) tuple so that exposures facing different counterparties are not netted.  Whether the ledger persists positions or computes them on demand from move history is an implementation detail; the smart contract is agnostic.
...
- Unit state: the lifecycle and contingent state of a unit, uniform across all holders.  Compound, with three parts: (i) a *liveliness* component (e.g. `Active`, `Matured`, `Expired`, optionally with contingent flags like `barrier_knocked`); (ii) a *last-lifecycle-event marker* recording the most recent lifecycle event applied to the unit (e.g. `Coupon paid 2016-10-01`); and (iii) a *corporate actions applied* list recording every corporate action that has been applied to the unit.  Written together as `Active | Coupon paid 2016-10-01 | CAs: [split 2018-06-01, split 2022-08-15]`.  Lifecycle events are scheduled upfront and applied in a defined order, so only the latest is kept; corporate actions are not generally known at inception and are order-independent across distinct events, so the full list is retained.  Both the marker and the CA list are consulted by the smart contract to enforce idempotent event delivery (see invariants).  A unit's state cannot differ by counterparty (e.g. a listed future cannot be `Active` for one party and `Matured` for another).
- Position state: a counter map keyed by (unit, wallet, counterparty wallet) whose values are quantities split by settlement bucket — `Settled`, `Pending(date)` (one sub-bucket per anticipated settlement date), and `Failed`.  Position state is supplied to the smart contract as input; it is the v1 representation of move-level settlement state, aggregated up to the position grain.  Individual smart contracts may extend position state with product-specific fields (e.g. futures carry a running cost basis scalar in addition to the bucketed counters).  See [invariants.md](docs/invariants.md) State Model.
