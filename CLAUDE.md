...
- Unit state: the lifecycle and contingent state of a unit, uniform across all holders.  Compound, with two parts: (i) a *liveliness* component (e.g. `Active`, `Matured`, `Expired`, optionally with contingent flags like `barrier_knocked`), and (ii) a *last-lifecycle-event marker* recording the most recent lifecycle event applied to the unit (e.g. `Coupon paid 2016-10-01`).  Written together as `Active | Coupon paid 2016-10-01`.  The marker is consulted by the smart contract to enforce idempotent event delivery (see invariants).  A unit's state cannot differ by counterparty (e.g. a listed future cannot be `Active` for one party and `Matured` for another).
