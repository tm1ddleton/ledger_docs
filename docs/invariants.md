...
13. **Provenance, not shape, distinguishes events**: Two events that produce the same economic effect have the **same ledger and CDM shape** regardless of how they arose. A move generated by a primary trigger (a QRL-delivered lifecycle event, a self-determined observation breach) and the same move generated by a reconciliation activity (mirroring an externally-delivered CDM event from a counterparty, replaying a corrected feed) are indistinguishable by shape; they differ only in **provenance metadata** carried alongside. Provenance records, at minimum, the source (`Primary` / `ReconciliationMirrored` / `Correction`), the originating system or actor, the upstream event identifier, and — where applicable — the resolution mode (e.g. `Automated` / `Override`). Provenance never changes which moves are produced or their amounts; it is the audit and de-duplication channel, not an economic input. Idempotency (invariant 10) keys off the upstream event identifier carried in provenance: a reconciliation-mirrored event whose identifier matches an already-applied primary event is a no-op, which is what makes internal generation and CDM-native counterparty interop safe to run against the same ledger. The corporate-action mode recorded on CA moves and the unit-state marker (see [Corporate Action Orchestration](#corporate-action-orchestration)) is the canonical worked example of provenance metadata.
...
### Exchange-Facing Book Is Structurally Flat

The internal allocation leg is part of the **same atomic transaction** as the external leg (point 3 above), and — for daily-margined products — each internal VM allocation is part of the same `DailySettlementEvent` as the single external VM payment (see the two-tier structure in [futures.md](smart_contracts/futures.md#vm-granularity-and-settlement-netting)). It follows as an invariant that **the exchange-facing book nets to zero on every trade and every VM cycle**: the sum of internal allocation moves always equals the external move in the opposite direction. The EFB carries exchange-facing settlement risk in flight but holds no residual position or P&L once both legs of a transaction reach the same settlement state.

This makes reconciliation a **single-wallet check**: the EFB should always net to flat, so any non-zero net position on it is, by construction, an unmatched or part-settled leg requiring investigation — there is no need to reconcile position-by-position across internal books to detect a break.
