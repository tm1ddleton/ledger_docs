...
The futures smart contract is stateless. Each invocation receives Product state, Unit state, and Position state per the global [State Model](../state.md).
...
In addition to the global bucketed counters (per [State Model](../state.md)), the futures position state for each `(unit, wallet, counterparty wallet)` tuple carries one extra scalar:
